  ...
  return (
    <div className={cn(
      ...
      <div className={cn("px-4 py-3 flex justify-between items-center border-b", estado.border, estado.bg)}>
        <span className="font-black text-xs tracking-wider opacity-80">#{order.id.slice(0, 8)}</span>
...
function StatsDashboard({ orders, totalSales, refresh }: { orders: Order[], totalSales: number, refresh: () => void }) {
  ...
  // Pedidos desde el último cierre (o desde medianoche si nunca se cerró)
  const startOfToday = new Date(); startOfToday.setHours(0, 0, 0, 0)
  const desde = lastCierre > startOfToday.getTime() ? lastCierre : startOfToday.getTime()
  const ordersToday = completedOrders.filter(o => new Date(o.createdAt).getTime() >= desde)

  // Cálculos de Caja — excluye mercadopago_pendiente (no confirmado)
  const totalEfectivo = ordersToday.filter(o => o.paymentMethod === "efectivo").reduce((sum, o) => sum + o.total, 0)
  const totalTransferencia = ordersToday.filter(o => o.paymentMethod === "transferencia").reduce((sum, o) => sum + o.total, 0)
  const totalMP = ordersToday.filter(o => o.paymentMethod === "mercadopago").reduce((sum, o) => sum + o.total, 0)
  const totalHoy = totalEfectivo + totalTransferencia + totalMP
  ...
  const handleCerrarCaja = async () => {
    ...
    try {
      const now = Date.now()
      localStorage.setItem("last_cierre", String(now))
      setLastCierre(now)

      const fecha = new Date(now).toLocaleString("es-AR")
      ...
      if (printWindow) {
        printWindow.document.write(`
          ...
          <style>
            ...
            h2 { margin: 0 0 4px; } p { margin: 0 0 2px; font-size: 12px; color: #555; }
            hr { margin: 12px 0; border: none; border-top: 1px dashed #aaa; }
            .row { display: flex; justify-content: space-between; margin: 6px 0; font-size: 14px; }
            .total { font-weight: bold; font-size: 16px; }
          </style></head><body>
          <h2>Smash Burger Alberdi</h2>
          <p>Cierre de caja</p>
          <p>${fecha}</p>
          <hr/>
          <div class="row"><span>Pedidos completados</span><span>${ordersToday.length}</span></div>
          <hr/>
          <div class="row"><span>Efectivo</span><span>$${totalEfectivo.toLocaleString("es-AR")}</span></div>
          <div class="row"><span>Transferencia</span><span>$${totalTransferencia.toLocaleString("es-AR")}</span></div>
          <div class="row"><span>MercadoPago</span><span>$${totalMP.toLocaleString("es-AR")}</span></div>
          <hr/>
          <div class="row total"><span>TOTAL</span><span>$${totalHoy.toLocaleString("es-AR")}</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#111] border border-white/5 p-6 rounded-2xl shadow-xl">
          ...
          <div className="flex gap-3 mt-2 flex-wrap">
            ...
            <div className="text-[10px] text-muted-foreground"><span className="text-sky-400">Transf:</span> ${totalTransferencia.toLocaleString("es-AR")}</div>
            ...
            <BarChart data={(() => {
              ...
              <Bar dataKey="total" radius={[6, 6, 0, 0]}>
                {Array.from({ length: 7 }, (_, i) => (
                  <Cell key={i} fill={i === 6 ? "hsl(var(--primary))" : "#333"} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
          ...
          <div className="bg-[#0a0a0a] border border-white/10 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-8 text-center">
              ...
              <div className="space-y-3 mb-8">
                ...
                <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                  <span className="text-sm font-medium text-muted-foreground">Transferencia</span>
                  <span className="font-black text-sky-400">${totalTransferencia.toLocaleString("es-AR")}</span>
                </div>
                <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                  <span className="text-sm font-medium text-muted-foreground">MercadoPago</span>
